...
from fairseq.data import Dictionary, PhonemeDictionary, encoders, iterators
from fairseq.data.audio.speech_text_triple_align_mbart_dataset import (
    SpeechTextTripleAlignMBARTDataset, 
    SpeechTextTripleAlignMBARTDatasetCreator
)
...
@register_task("speech_to_text_triplet_align_with_extra_mt_mbart")
class SpeechToTextTripletAlignWithExtraMTMBARTTask(LegacyFairseqTask):
    @staticmethod
    def add_args(parser):
    ...
    def setup_task(cls, args, **kwargs):
        ...
        codes = SpeechTextTripleAlignMBARTDataset.get_lang_codes(os.path.join(args.mbart50_dir, 'ML50_langs.txt'))
        def load_dict(vocab_filename):
            ...
            for code in codes:
                _dict.add_symbol(SpeechTextTripleAlignMBARTDataset.LANG_TAG_TEMPLATE.format(code[:2].lower()))
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        st_dataset = SpeechTextTripleAlignMBARTDatasetCreator.from_tsv(
            self.args.data,
            self.data_cfg,
            split,
            self.src_dict,
    ...
    def get_batch_iterator(
        ...
        if not isinstance(dataset, MultiModalityDataset):
            return super(SpeechToTextTripletAlignWithExtraMTMBARTTask, self).get_batch_iterator(
                dataset,
                max_tokens,
                max_sentences,
                max_positions,
    ...
    def build_model(self, args):
        ...
        model = super(SpeechToTextTripletAlignWithExtraMTMBARTTask, self).build_model(args)
    ...
    def build_generator(
        ...
        lang_token_ids = {
            ...
            if SpeechTextTripleAlignMBARTDataset.is_lang_tag(s)
        }

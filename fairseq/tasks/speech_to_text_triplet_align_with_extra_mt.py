...
from fairseq.data.audio.speech_text_triple_align_dataset import (
    ...
    SpeechTextTripleAlignDatasetCreator
)
...
@register_task("speech_to_text_triplet_align_with_extra_mt")
class SpeechToTextTripletAlignWithExtraMTTask(LegacyFairseqTask):
    @staticmethod
    def add_args(parser):
    ...
    def setup_task(cls, args, **kwargs):
        ...
        src_dict = None
        src_dict_path = op.join(args.data, data_cfg.phoneme_vocab_filename)
        if op.isfile(src_dict_path):
            src_dict = PhonemeDictionary.load(src_dict_path)
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        st_dataset = SpeechTextTripleAlignDatasetCreator.from_tsv(
            self.args.data,
            self.data_cfg,
            split,
            self.src_dict,
    ...
    def get_batch_iterator(
        ...
        if not isinstance(dataset, MultiModalityDataset):
            return super(SpeechToTextTripletAlignWithExtraMTTask, self).get_batch_iterator(
                dataset,
                max_tokens,
                max_sentences,
                max_positions,
    ...
    def build_model(self, args):
        ...
        model = super(SpeechToTextTripletAlignWithExtraMTTask, self).build_model(args)

        self.static_model = super(SpeechToTextTripletAlignWithExtraMTTask, self).build_model(args)
    ...
    def build_generator(
        ...
        lang_token_ids = {
            ...
            if SpeechTextTripleAlignDataset.is_lang_tag(s)
        }
    ...
    def begin_epoch(self, epoch, model):
        ...
        self.static_model.load_state_dict(model.state_dict())

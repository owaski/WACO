...
@register_model("xstnet_nllb")
class XSTNetNLLB(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            type=str, 
            metavar='STR',
            help='directory to nllb model'
        )
        parser.add_argument(
            '--n-freeze-speech-encoder-layer',
            type=int,
            default=0,
        )
    ...
    def build_encoder(cls, args, dict, embed_tokens):
        encoder = XSTNetNLLBEncoder(args, dict, embed_tokens)
    ...
    def build_model(cls, args, task):
        ...
        nllb = torch.load(os.path.join(args.nllb_dir, 'nllb200densedst600mcheckpoint.pt'))
        xstnet_nllb_base_architecture(args)

        args_dict = args.__dict__
        args_dict.update(nllb['cfg']['model'].__dict__)
        args_dict['_name'] = 'xstnet_nllb_base'
        ...
        nllb_encoder_params = {k[8:]: v for k, v in nllb['model'].items() if k.startswith('encoder.')}
        nllb_decoder_params = {k[8:]: v for k, v in nllb['model'].items() if k.startswith('decoder.')}
        encoder.transformer_encoder.load_state_dict(nllb_encoder_params)
        decoder.load_state_dict(nllb_decoder_params)
...
class XSTNetNLLBEncoder(FairseqEncoder):
    ...
    def _build_acoustic_encoder(self, args):
        ...
        self.w2v_args = Namespace(**ckpt['cfg']['model'])
        if not self.use_asr_finetune_w2v:  # if use ssl-trained only
            # self.w2v_args = ckpt["args"]
            self.wav2vec_model = Wav2Vec2Model.build_model(self.w2v_args, task=None)
        ...
        for i in range(args.n_freeze_speech_encoder_layer):
            freeze_module_params(self.wav2vec_model.encoder.layers[i])
...
@register_model_architecture(model_name="xstnet_nllb", arch_name="xstnet_nllb_base")
def xstnet_nllb_base_architecture(args):
    # wav2vec2
